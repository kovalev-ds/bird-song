...
import { createCandidateCard } from "./components/candidate.js";
...
const createGame = (matrix, points = 5) => {
  const mapCandidates = (list) =>
    list.map((item) => ({ ...item, isTried: false }));
  ...
  const app = observable({
    ...
    isSolved() {
      if (!isSolved) {
        isSolved = this.mystery.id === this.candidate?.id;
      }
...
G_A_M_E.listen("candidate", ({ mystery, candidate, candidates }) => {
  ...
  if (isSolved && mystery.id === candidate?.id) {
...
const mysteryCard = createMysteryCard(G_A_M_E.mystery);
const candidateCard = createCandidateCard(G_A_M_E.candidate);
...
const scoreUiBlock = createElement("div", { text: G_A_M_E.score || "0" });
...
G_A_M_E.listen("candidate", ({ candidate, mystery }) => {
  const isSolved = G_A_M_E.isSolved();

  nextRoundButton.update(isSolved);
  candidateCard.update(candidate);

  if (isSolved && mystery.id === candidate?.id) {
    mysteryCard.update(candidate);
  }
});
